...
class CartItem(object):
    ...
    """
    def __init__(self, product, quantity, price, configuracao_pacote=None, quantidade=None, pacote=None):
        self.product = product
        self.quantity = int(quantity)
        self.price = Decimal(str(price))
        self.configuracao_pacote = configuracao_pacote
    ...
    def to_dict(self):
        return {
            ...
            'configuracao_pacote': self.configuracao_pacote,
...
class Cart(object):
    ...
    def __init__(self, session, session_key=None):
        ...
        if self.session_key in self.session:
            ...
            try:
                ids_in_cart = cart_representation['items'].keys()
                self.discount = cart_representation['discount']
                self.cupom = cart_representation['cupom']
                self.cpf = cart_representation['cpf']
                self.tax = cart_representation['tax']
            except KeyError:
                self.update_session()
                cart_representation = self.session[self.session_key]
                ids_in_cart = cart_representation['items'].keys()
            ...
            for product in products_queryset:
                item = cart_representation['items'][str(product.pk)]
                self._items_dict[product.pk] = CartItem(
                    product, item['quantity'],
                    Decimal(item['price']),
                    item['configuracao_pacote'],
                    item['quantidade'],
                    item['pacote']
    ...
    def add(self, product, price=None, quantity=1, configuracao_pacote=False, quantidade=False, pacote=False):
        """
        ...
        if product in self.products:
            ...
            self._items_dict[product.pk] = CartItem(
                product, quantity, price, configuracao_pacote, quantidade, pacote
            )
        self.update_session()
    ...
    def cart_serializable(self):
        ...
        for item in self.items:
            ...
            cart_representation['items'][product_id] = item.to_dict()
