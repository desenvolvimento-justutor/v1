...
from signals import (notificacao_recebida, NOTIFICATION_STATUS, checkout_realizado, checkout_realizado_com_sucesso,
...
class PagSeguroApi(object):
    ...
    def get_notification(self, notification_id):
        logger.debug('[GET_NOTIFICATION] id: {}'.format(notification_id))
        ...
        logger.debug('[GET_NOTIFICATION] status code: {}'.format(response.status_code))

        if response.status_code == 200:
            root = xmltodict.parse(response.text)
            transaction = root['transaction']
            logger.debug('[GET_NOTIFICATION] transaction: {}'.format(transaction))
