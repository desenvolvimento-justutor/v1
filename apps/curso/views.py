...
def extrair_nota(texto):
    nota = None
    padrao = r"<strong>A SUA NOTA FINAL FOI:\s*([\d,]+)</strong>"
    match = re.search(padrao, texto)
    if match:
        nota = match.group(1)
        try:
            nota = Decimal(nota.replace(",", "."))
        except Exception as e:
            logger.error("ERRO AO EXTRAIR A NOTA: %s", e)
            nota = None
    return nota
...
def create_checkout(curso, aluno, request=None):
    ...
    Transaction.objects.create(
        code=checkout_obj.code, checkout=checkout_obj, status="pago"
    )
    CheckoutItens.objects.create(checkout=checkout_obj, curso=curso, qtda=1, valor=0)
    if request:
        msg = (
            "Voce adquiriu gratuitamente o(a)</br></br> {}: {}.</br></br>"
...
def relatorio(request):
    ...
    for chk in chks:
        ...
        if vlr_desc > 1:
            vlr_desc *= -1
        desc += vlr_desc
        txt += "{0:80s} {1:15s} {2} {3:7f} {4:10s}\n".format(
            "{0} ({1})".format(aluno_nome, aluno_email),
            chk.cpf or "",
            chk.date.strftime("%d/%m/%Y"),
            chk.total,
            transc.get_status_display(),
        )
        txt += "{0:107s} {1:7.2f} {2:10.2f}\n".format(
...
def categoria(request, slug):
    ...
    if request.method == "POST":
        ...
        return HttpResponseRedirect(
            reverse("curso:categoria", args=[obj_categoria.slug])
        )

    now = timezone.now()
    ocursos = Curso.objects.filter(categoria=obj_categoria, aluno__isnull=True)
    context = {"categoria": obj_categoria, "cursos": ocursos, "cursos_inativos": True}
    ...
    if obj_categoria.tipo in ["S", "O", "D"]:
        ...
        if obj_categoria.tipo == "O":
            ...
            # TIPOS PECA
            tipos_peca = (
                TipoPecaPratica.objects.filter(**kwargs)
                .values("id", "nome")
                .annotate(t=Count("id"))
            )
            context["tipos_peca"] = tipos_peca
            professores = (
                Professor.objects.filter(**kwargs)
                .values("id", "nome")
                .annotate(t=Count("id"))
            )
            context["professores"] = professores
        elif obj_categoria.tipo == "D":
            kwargs = {"simulado__isnull": False}
            context.update(
                ...
                .values("id", "nome")
                .annotate(t=Count("id")),
                cargos=Cargo.objects.filter(simulado__curso__categoria__tipo="D")
                .values("id", "nome")
                .annotate(t=Count("id")),
                concursos=Concurso.objects.filter(simulado__curso__categoria__tipo="D")
                .values("id", "nome")
                .annotate(t=Count("id")),
            ...
            # ESFERAS_ESPECIFICAS
            esferas_especificas = (
                EsferaEspecifica.objects.filter(**kwargs)
                .values("id", "nome")
                .annotate(t=Count("id"))
            )
            ...
            # TIPOS PROCEDIMENTO
            tipos_procedimento = (
                TipoProcedimento.objects.filter(**kwargs)
                .values("id", "nome")
                .annotate(t=Count("id"))
            )
        ...
        if obj_categoria.tipo != "D":
            # DISCIPLINAS
            disciplinas = (
                Disciplina.objects.filter(**kwargs)
                .values("id", "nome")
                .annotate(t=Count("id"))
            )
...
def curso_gratis(request, slug):
    ...
    videos = obj_curso.videogratis_set.filter(
        Q(data_ini__lte=timezone.now()) | Q(data_ini=None)
    )
    context = {
        "curso": obj_curso,
        "videos": videos,
...
def curso(request, slug):
    ...
    cursos = Curso.objects.filter(categoria=obj_curso.categoria).exclude(
        id=obj_curso.id
    )

    aval_otimo = CursoAvaliacao.objects.filter(
        curso_id=obj_curso.id, avaliacao="O"
    ).count()
    aval_bom = CursoAvaliacao.objects.filter(
        curso_id=obj_curso.id, avaliacao="B"
    ).count()
    aval_ruim = CursoAvaliacao.objects.filter(
        curso_id=obj_curso.id, avaliacao="R"
    ).count()
    try:
        objcheckout = CheckoutItens.objects.filter(
            curso=obj_curso, checkout__aluno=request.user.aluno
        ).first()
    except:
        objcheckout = None
    context = {
...
def livro(request, slug):
    ...
    aval_otimo = CursoAvaliacao.objects.filter(
        curso_id=obj_livro.id, avaliacao="O"
    ).count()
    aval_bom = CursoAvaliacao.objects.filter(
        curso_id=obj_livro.id, avaliacao="B"
    ).count()
    aval_ruim = CursoAvaliacao.objects.filter(
        curso_id=obj_livro.id, avaliacao="R"
    ).count()
    try:
        objcheckout = CheckoutItens.objects.filter(
            curso=obj_livro, checkout__aluno=request.user.aluno
        ).first()
    except:
        objcheckout = None
...
def curso_sentenca(request, slug):
    ...
    if request.method == "POST":
        ...
        return HttpResponseRedirect(
            reverse("curso:curso-sentenca", args=[obj_curso.slug])
        )
...
def get_payment_methods(request):
    ctx = {}
    if request.method == "POST":
        ctx = {"pgtos": request.POST.get("payments")}
...
def carrinho_pagamento(request):
    ctx = {"aluno": request.user.aluno, "estados": ESTADOS_BRASILEIROS_NOMES}
...
def checkout(request):
    ...
    for i in cart.items:
        item = PagSeguroItem(
            ...
            description="{0}".format(i.product.nome),
    ...
    if not success:
        ...
        ret.update({"link": trans.get("paymentLink", ""), "code": data_api.get("code")})
        chk = Checkout(
        ...
        for i in cart.items:
            item = CheckoutItens(
                checkout=chk, curso=i.product, valor=i.price, qtda=i.quantity
            )
            item.save()
            p = item.product
            if p.aluno:
...
def get_amostra_json(request):
    ...
    if request.GET.get("tipo") == "resposta":
        ...
        vals["conteudo"] = "{0}".format(st.resposta)
    else:
        try:
            ...
            amostra = st.amostra
            vals["conteudo"] = "{0}".format(st.conteudo)
        except Exception as e:
...
def post_resposta(request, pk):
    ...
    try:
        ...
        if not any([tarefa.arquivo, len(tarefa.resposta)]):
            msg = (
                "Atividade em branco! Você não escreveu nada no editor de textos e nem anexou arquivo com sua"
            ...
            if data.get("concluido") == "true" and not tarefa.concluido:
                ...
                if atividade.enable_gpt:
                    ...
                    if correcao:
                        ...
                        tarefa.gabarito = correcao
                        tarefa.nota = extrair_nota(correcao)
                        tarefa.gpt_result = gpt_api.gpt_result
                tarefa.save()
                ...
                if limitar_correcao and tarefas_concluidas.count() == limitar_correcao:
                    ...
                    msg = (
                        "Você já respondeu ao número máximo de atividades com direito à correção individual, sendo "
    ...
    return JsonResponse({"msg": msg, "flag": flag, "finalizar": finalizar})
...
def post_correcao(request, pk):
    ...
    try:
        ...
        if data.get("concluido") == "true":
            ...
            tarefa.save()
            enviar_email(
                "curso/email/correcao-enviada.html",
...
def post_sentenca(request, pk):
    ...
    try:
        ...
        if data.get("concluido") == "true":
            ...
            enviar_email(
                "curso/email/sentenca-finalizada.html",
                "Nova sentença disponível para correção",
                [sentenca.sentenca_avulsa.professor.user.email],
                ead=True,
            )
        sentenca.save()
    except Exception as e:
...
def post_sentenca_oab(request, pk):
    ...
    try:
        ...
        if data.get("concluido") == "true":
            sentenca.status = "A"
            enviar_email(
                "curso/email/sentenca-oab-finalizada.html",
...
def atividade_responder(request, pk):
    ...
    aluno = request.user.aluno
    try:
        tarefa = TarefaAtividade.objects.get(atividade=atividade, aluno=aluno)
    except Exception as e:
        messages.info(request, str(e))
        tarefa = TarefaAtividade(atividade=atividade, aluno=aluno)
        tarefa.save()
    if request.method == "POST":
...
def sentenca_responder(request, pk):
    ...
    ctx = {"titulo": sentenca, "menu": "Atividade Avulsa", "sentenca": sentenca}
...
def sentenca_oab_responder(request, pk):
    ...
    ctx = {"titulo": sentenca, "menu": "OAB 2ª Fase", "sentenca": sentenca}
...
def imprimir(request, pk):
    if request.GET.get("action") == "resposta":
        ...
        ctx = {"conteudo": tarefa.resposta, "titulo": tarefa.atividade}
        ...
        ctx = {"conteudo": atividade.tarefa, "titulo": atividade}
...
def baixar_correcao(request, pk):
    ...
    if action == "gabarito_atv":
        ...
        tarefa = get_object_or_404(TarefaAtividade, id=pk)
        atividade = tarefa.atividade
    ctx.update({"tarefa": tarefa, "atividade": atividade, "titulo": atividade})
...
def sentenca_imprimir(request, pk):
    ...
    ctx = {"conteudo": conteudo, "titulo": titulo}
    # template = get_template('imprimir-atividade.html')
    # context = Context(ctx)
    # html = template.render(context)
    # result = StringIO.StringIO()
...
def download_tarefa(request, pk):
    ...
    response["Content-Disposition"] = 'attachment; filename="T{0:03d}-{1}.html"'.format(
        pk, slugify(aluno)
    )
    html = """
    <!DOCTYPE html>
...
def enviar_correcao(request):
    ...
    try:
        if data.get("action") == "sentenca":
            ...
            sentenca.status = "C"
            sentenca.save()
            messages.success(request, "Sentença enviada com sucesso")
            enviar_email(
                "curso/email/sentenca-corrigida.html",
                "Sua sentença já foi corrigida!",
                [sentenca.aluno.email],
            ...
            sentenca.status = "C"
            sentenca.save()
            messages.success(request, "Peça enviada com sucesso")
            enviar_email(
                "curso/email/sentenca-corrigida-oab.html",
                "Sua peça já foi corrigida!",
...
def montar_pacote(request):
    context = {"titulo": "Pacote personalizado"}
    template_name = "curso/montar_pacote_list.html"
    filtro = dict(categoria__tipo="S")
    # ESFERAS_ESPECIFICAS
    esferas_especificas = (
        EsferaEspecifica.objects.filter(sentencaavulsa__isnull=False)
        .values("id", "nome")
        .annotate(t=Count("id"))
    )
    context["esferas_especificas"] = esferas_especificas
    # TIPOS PROCEDIMENTO
    tipos_procedimento = (
        TipoProcedimento.objects.filter(sentencaavulsa__isnull=False)
        .values("id", "nome")
        .annotate(t=Count("id"))
    )
    context["tipos_procedimento"] = tipos_procedimento
    # DISCIPLINAS
    disciplinas = (
        Disciplina.objects.filter(sentencaavulsa__isnull=False)
        .values("id", "nome")
        .annotate(t=Count("id"))
    )
    context["disciplinas"] = disciplinas
    # DISCIPLINAS
    disciplinas = (
        Categoria.objects.filter(tipo="S").values("id", "nome").annotate(t=Count("id"))
    )
        ...
        if request.method == "POST":
            ...
            if action == "add" and curso_pk:
                if not combo_aluno:
                    ...
                    combo_aluno.save()
                    title = "Pacote personalizado #{:04d}".format(combo_aluno.pk)
                ...
                msg = "{} adicionado ao pacote".format(qs_curso.nome)
                if desconto_porcento:
                    msg += (
                        '<div class="alert alert-success" role="alert" style="margin-top: 10px;">'
                        '<small style="text-align: justify">Você já conseguiu {}% de desconto, economizando R$ {}.'
                        " Quanto mais atividades você escolher, maior será o seu desconto.</small></div>".format(
                            moeda_nosymbol(desconto_porcento),
                            moeda_nosymbol(desconto_valor),
                        )
                    )
                messages.success(request, msg)
            elif action == "remove":
                ...
                messages.error(request, "{} removido do pacote".format(qs_curso.nome))
...
def ajax_search_simulado(request):
    cursos = Curso.objects.filter(
        nome__icontains=request.GET["q"], simulado__isnull=False
    ).values("nome")
...
def ajax_gerar_cortesia(request):
    ...
    try:
        ...
        if emails:
            ...
            for email in emails.split(","):
                try:
                    ...
                    try:
                        ...
                        enviar_email_old(
                            "curso/email/cortesia.html",
                            "Seu código de cortesia já está disponível!",
                            [email],
                            ctx,
                        )
                except ValidationError:
                    status = 500
                    message = 'Email "%s" invalido' % email
            response = JsonResponse(data={"message": message}, status=status)
            response.reason_phrase = message
            ctx = {"curso": c}
            if cadastrados:
                enviar_email(
                    "curso/email/cortesia-cadastrado.html",
                    "Seu acesso de cortesia já está liberado!",
                    cadastrados,
                    ctx,
                )
            return response
            ...
            create()
            return JsonResponse({"message": "%s cortesia(s) gerada(s)!" % data["q"]})
    except Exception as e:
        response = JsonResponse(
            data={"status": "false", "message": "message"}, status=500
        )
