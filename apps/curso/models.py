...
class Categoria(models.Model):
    ...
    titulo = models.CharField(
        ...
        blank=True,
        null=True,
        help_text="Título que será exibido no menu. Caso não informe será exibido o 'Nome'.",
    )
...
class Autor(models.Model):
    ...
    sobre = models.TextField(verbose_name="Sobre", blank=True, null=True)
...
class Curso(models.Model):
    ...
    limitar_correcao = models.SmallIntegerField(
        verbose_name="Limitar correções",
        default=0,
    ...
    sentenca_oab = models.OneToOneField(
        verbose_name="OAB 2ª Fase", blank=True, null=True, to="SentencaOAB"
    )
    simulado = models.OneToOneField(
        verbose_name="Simulado", to=Simulado, blank=True, null=True
    )
    nome = models.CharField(verbose_name="Título", max_length=250)
    video = FileBrowseField(
        verbose_name="Vídeo Demonstração",
        max_length=200,
        extensions=[".mp4"],
        blank=True,
        null=True,
        help_text="Selecione um vídeo no fotmat .MP4",
        directory="videos/",
    )
    thumbnail = ImageField(
        verbose_name="Capa",
        upload_to="videos_thumb/",
        blank=True,
        null=True,
        help_text="Miniatura que será exibido, se não escolher será gerado automaticamente",
    )
    imagem = ImageField(
        verbose_name="Imagem",
        blank=True,
        null=True,
        upload_to="img_thumb/",
        help_text="Selecione um imagem de capa que será exibida na listagem dos curso (Tam. 270x170).",
    )
    descricao = models.TextField(verbose_name="Apresentação")
    valor = models.DecimalField(verbose_name="Valor", max_digits=15, decimal_places=2)
    disponivel = models.BooleanField(verbose_name="Turma Disponível?", default=True)
    inicio_gratis = models.BooleanField(
        verbose_name="Início Grátis?",
        default=False,
        help_text="Informe caso o curso tenha início grátis.",
    )
    data_ini = models.DateTimeField(
        ...
        "colocará o curso no site",
    )
    data_fim = models.DateTimeField(
        verbose_name="Data final",
        blank=True,
        null=True,
        help_text="A partir dessa data o sistema automaticamente irá marcar o curso "
        "como encerrado",
    )
    professores = models.ManyToManyField("professor.Professor", blank=True)
    blocos = models.ManyToManyField(verbose_name="Quizz", to="quizz.Bloco", blank=True)
    saiba_mais = models.TextField(verbose_name="Saiba+", null=True, blank=True)
    cronograma = models.TextField(verbose_name="Cronograma", null=True, blank=True)
    mural = models.TextField(verbose_name="Mural", null=True, blank=True)
    ...
    certificado = models.TextField(verbose_name="Certificado", blank=True, null=True)
    ...
    meta_description = models.CharField(
        ...
        null=True,
        max_length=160,
        help_text="Resumo geral do site. Frase que descreva muito bem o Curso. Quando"
    ...
    meta_keywords = models.CharField(
        ...
        null=True,
        max_length=100,
        help_text="Procure usar umas poucas palavras que descrevam o conteúdo do Curso. "
    ...
    edicao = models.SmallIntegerField(verbose_name="Edição", blank=True, null=True)
    ano = models.SmallIntegerField(verbose_name="Ano", blank=True, null=True)
    paginas = models.SmallIntegerField(
        verbose_name="Páginas", blank=True, null=True, help_text="Quantidade de Páginas"
    )
    # combo
    ...
    aluno = models.ForeignKey(
        verbose_name="Aluno", to="aluno.Aluno", blank=True, null=True
    )
    status = models.CharField(
        verbose_name="Status",
        max_length=1,
        choices=[("A", "Aberto"), ("C", "Carrinho"), ("F", "Finalizado")],
        default="A",
    )
    omie_id = models.BigIntegerField(verbose_name="Código Omie", blank=True, null=True)
    is_video_curso = models.BooleanField(
        verbose_name="Curso em Vídeo",
        default=False,
        help_text="Informe se o curso é em primordialmente em vídeo",
    )
    is_tutorial = models.BooleanField(verbose_name="Tutorial", default=False)
    ...
    def incluir_cadastro_servico(self):
        ...
        if not self.omie_id:
            try:
                result = api.incluir_cadastro_servico(
                    descricao=self.nome, codigo=self.pk, preco_unit=self.valor
                )
    ...
    def matriculados(self):
        chk = self.checkoutitens_set.filter(checkout__transaction__status="pago")
        return chk.count()
    ...
    def create_screenshot(self):
        ...
        if video_file:
            png_filename = "{0}.png".format(video_file.filename)
            png_file = os.path.join(MEDIA_ROOT, "videos_thumb", png_filename)
            if not os.path.exists(png_file):
                command = "ffmpeg -y -i '{0:s}' -ss 00:00:05 -vframes 1 '{1:s}'".format(
                    video_file.path_full, png_file
                )
...
class Cortesia(models.Model):
    ...
    utilizado = models.BooleanField(verbose_name="Utilizado?", default=False)
...
class ComboAluno(Curso):
    ...
    def calc(self, safe=False):
        ...
        if pact_igual:
            msg.append(
                'O seu desconto atual é de <b class="text-primary">{:.2f}%</b>.'.format(
                    pact_igual.desconto
                )
            )
        ...
        if pact_maior:
            ...
            msg.append(
                'Adicione mais <b class="text-primary">{}</b> sentença(s) e seu desconto será de'
                ' <b class="text-primary">{:.2f}%</b>.'.format(
...
class Discussao(models.Model):
    ...
    def status(self):
        ...
        if dini <= now <= dfim:
            ...
            ret = "Aguarde"
            label = "warning"
        return dict(status=ret, label=label)
...
def com1(sender, **kwargs):
    ...
    if parent:
        ...
        enviar_email(
            ...
            [aluno.email],
            {"discuss": discuss, "comentario": comment},
        )
...
class PdfModulo(models.Model):
    ...
    modulo = models.ForeignKey(Modulo, verbose_name="Módulo", blank=True, null=True)
    ...
    pdf = FileBrowseField(
        ...
        blank=True,
        null=True,
        help_text="Selecione um arquivo no fotmat .PDF",
    )
...
class Atividade(models.Model):
    ...
    descricao = models.TextField(
        ...
        blank=True,
        null=True,
        help_text="Breve descrição da atividade",
    )
    gabarito = models.TextField(verbose_name="Gabarito", blank=True, null=True)
    data = models.DateField(verbose_name="Data de Criação", default=datetime.today)
    data_ini = models.DateTimeField(verbose_name="Data de início", null=True)
    data_fim = models.DateTimeField(verbose_name="Data de término", null=True)
    tipo_retorno = models.CharField(
        verbose_name="Tipo de retorno do professor",
        max_length=1,
        choices=[
    ...
    tarefa = models.TextField(verbose_name="Tarefa", blank=True, null=True)
    ...
    resolucao_obrigatorio = models.BooleanField(
        verbose_name="Resolução Obrigatória?",
        default=False,
    ...
    parametros_correcao = models.TextField(
        verbose_name="Parâmetros de Correção", blank=True, null=True
    )
    exemplos_correcao = models.TextField(
    ...
    temperature = models.DecimalField(
        ...
        decimal_places=2,
        default=0.7,
        help_text="Define a aleatoriedade da resposta. Valores entre 0 e 1 são aceitos. Ex.: 0.50",
    )
    ...
    def get_resposta_padrao(self):
        ...
        if (
            self.tipo_retorno == "R"
            and self.resposta_padra
            and self.resposta_padrao_data
        ):
    ...
    def tipo_retorno_label(self):
        return {"F": "info", "R": "primary", "C": "warning"}.get(self.tipo_retorno)
...
class AtividadeModelo(models.Model):
    ...
    arquivo = models.FileField(verbose_name="Arquivo", upload_to="atividade-modelo")
...
class TarefaAtividade(models.Model):
    ...
    professor = models.ForeignKey(
        verbose_name="Corrigido por", to="professor.Professor", blank=True, null=True
    )
    gabarito = models.TextField(
        verbose_name="Gabarito",
        blank=True,
        null=True,
        help_text="Correção a partir do gabarito.",
    )
    corrigido = models.BooleanField(verbose_name="Corrigido?", default=False)
    limitada = models.BooleanField(verbose_name="Limitada", default=False)
    arquivo = models.FileField(
        verbose_name="Selecione o arquivo",
        upload_to="atividades",
        blank=True,
        null=True,
    )
    gpt_result = models.TextField(null=True, blank=True)
    nota = models.DecimalField(max_digits=9, decimal_places=2, blank=True, null=True)
    ...
    def render_gabarito(self):
        ...
        try:
            tabela_correcao = TabelaCorrecaoAluno.objects.get(
                formulario__atividade=tarefa_atividade.atividade,
                aluno=tarefa_atividade.aluno,
            )
            texto = "<h3><center>CORREÇÃO INDIVIDUALIZADA</center></h3>"
            texto += hr
            texto += tarefa_atividade.atividade.formulario.texto
            texto += br
            texto += '<table style="border: 1px solid black;">'
            texto += '    <tr style="border: 1px solid black;">'
            texto += '        <th style="border: 1px solid black;" colspan="3">Correção individualizada</th>'
            texto += "    </tr>"
            for tabela in tabela_correcao.tabelas.all():
                texto += '    <tr style="border: 1px solid black;">'
                texto += '        <td style="border: 1px solid black;"><center>Item</center></td>'
                texto += '        <td style="border: 1px solid black;"><center>Valor</center></td>'
                texto += '        <td style="border: 1px solid black;"><center>Nota</center></td>'
                texto += "    </tr>"

                txt = "<h4><strong>%s</strong></h4>" % tabela.tabela.item
                ...
                texto += '    <tr style="border: 1px solid black;">'
                texto += (
                    '        <td style="border: 1px solid black; padding: 0px 15px 15px 15px;">%s</td>'
                    % txt
                )
                texto += (
                    '        <td style="padding-top: 20px; border: 1px solid black; font-size: 1em" valign="top"><center><strong>%.02f</strong></center></td>'
                    % tabela.tabela.valor
                )
                texto += (
                    '        <td style="padding-top: 20px; border: 1px solid black; font-size: 1em" valign="top"><center><strong>%.02f</strong></center></td>'
            ...
            if tabela_correcao.texto:
                texto += br
                texto += "<h4><strong>Comentário final do professor:</strong></h4>"
                texto += tabela_correcao.texto
            texto += br
            texto += (
                "<h4>Nota obtida: <strong>%.02f</strong></h4>"
                % tabela_correcao.total().get("nota")
            )
            self.gabarito = texto
            self.save()
...
class Certificado(models.Model):
    ...
    def __unicode__(self):
        return "{0}/{1}".format(self.curso, self.aluno)
...
class DocCurso(models.Model):
    ...
    data_ativo = models.DateField(
        ...
        blank=True,
        null=True,
        help_text="Data em que arquivo será disponibilizado",
    )
    order = models.PositiveIntegerField(verbose_name="Ordem")
...
class VideoModulo(models.Model):
    ...
    descricao = models.CharField(
        ...
        blank=True,
        null=True,
        help_text="ID do vídeo (VDOCipher/Youtube)",
    )
    video = FileBrowseField(
        verbose_name="Vídeo",
        max_length=200,
    ...
    tipo = models.CharField(
        ...
        max_length=1,
        default="p",
        choices=[("p", "Padrão"), ("v", "VdoCipher"), ("y", "Youtube")],
    )
    order = models.PositiveIntegerField(verbose_name="Ordem")
    ...
    def create_screenshot(self):
        ...
        png_file = os.path.join(MEDIA_ROOT, "videos_thumb", png_filename)
        if not os.path.exists(png_file):
            command = "ffmpeg -y -i '{0:s}' -ss 00:00:01 -vframes 1 '{1:s}'".format(
                video_file.path_full, png_file
            )
            subprocess.call(shlex.split(command))
        self.thumbnail = os.path.join("videos_thumb", png_filename)
...
class Destaque(models.Model):
    ...
    data_ini = models.DateTimeField(
        ...
        default=django.utils.timezone.now,
        help_text="A partir dessa data o sistema automaticamente "
        "colocará o curso em destaque",
    )
    data_fim = models.DateTimeField(
        verbose_name="Data final",
        blank=True,
        null=True,
        help_text="A partir dessa data o sistema automaticamente irá retirar o curso "
        "em destaque",
    )
    # Sortable property
    order = models.PositiveIntegerField(verbose_name="Ordem")
...
class CursoGratis(models.Model):
    class Meta:
        verbose_name = "Curso Grátis"
        verbose_name = "Cursos Grátis"
        unique_together = ("nome",)

    serie = models.ForeignKey(verbose_name="Serie", to=Serie)
    ...
    thumbnail = ImageField(
        ...
        blank=True,
        null=True,
        help_text="Miniatura que será exibido.",
    )
    video = models.URLField(
        verbose_name="Vídeo URL", help_text="Vídeo de Apresentação."
    )
    descricao = models.TextField(verbose_name="Apresentação")
    data_cadastro = models.DateTimeField(
        verbose_name="Data ínicio", default=django.utils.timezone.now, editable=False
    )
...
class SentencaAvulsa(models.Model):
    ...
    titulo = models.CharField(
        verbose_name="Título", max_length=150, help_text="Identificação"
    )
    cod_youtube = models.CharField(
        verbose_name="Cód. Youtube",
        max_length=50,
        default="0",
    ...
    esfera_especifica = models.ForeignKey(
        verbose_name="Esfera Específica", to=EsferaEspecifica
    )
    tipo_procedimento = models.ForeignKey(
        verbose_name="Tipo de Procedimento", to=TipoProcedimento, null=True
    )
    ...
    amostra = models.TextField(
        verbose_name="Amostra da Sentença", blank=True, null=True
    )
    conteudo = models.TextField(
        verbose_name="Conteúdo", help_text="Conteúdo integral da proposta de sentença"
    )
    ...
    def span_nivel(self):
        ...
        if n == "F":
            ...
            s = "danger"
        return '<span class="label label-{0}">{1}</span>'.format(
            s, self.get_nivel_display()
        )
...
class SentencaOAB(models.Model):
    ...
    tipo_peca = models.ForeignKey(
        verbose_name="Tipo de Peça Prática", to=TipoPecaPratica
    )
...
class SentencaAvulsaAluno(models.Model):
    ...
    correcao_individual = models.TextField(
        verbose_name="Correção individual", blank=True, default=""
    )
    status = models.CharField(
        verbose_name="Status",
        max_length=1,
        default="I",
        choices=[("I", "Iniciada"), ("A", "Aguardando Correção"), ("C", "Corrigido")],
    )
    tempo = models.PositiveSmallIntegerField(verbose_name="Tempo", default=0)
    data_criacao = models.DateTimeField(
        verbose_name="Data de criação", editable=False, blank=True, auto_now_add=True
    ...
    @property
    def cod(self):
        return "T{0:03d}-{1}".format(self.id, self.sentenca_avulsa)
    ...
    def span_status(self):
        st = {"I": "success", "A": "danger", "C": "primary"}.get(self.status)
        return '<span class="label label-{0}">{1}</span>'.format(
...
class SentencaOABAvulsaAluno(models.Model):
    ...
    sentenca_oab = models.ForeignKey(verbose_name="OAB 2ª Fase", to=SentencaOAB)
    aluno = models.ForeignKey(verbose_name="Aluno", to="aluno.Aluno")
    resposta = models.TextField(verbose_name="Resposta", blank=True, default="")
    status = models.CharField(
        verbose_name="Status",
    ...
    def cod(self):
        try:
            return "T{0:03d}-{1}".format(self.id, self.sentenca_oab)
        except:
            return "T{0:03d}".format(self.id)
...
class LiberarCompraCurso(models.Model):
    ...
    curso = models.ForeignKey(verbose_name="Curso", to=Curso, on_delete=models.CASCADE)
    aluno = models.ForeignKey(
        verbose_name="Aluno", to="aluno.Aluno", on_delete=models.CASCADE
    )
