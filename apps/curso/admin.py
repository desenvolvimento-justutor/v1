...
class CortesiaForm(ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if instance:
            self.fields["codigo"].widget.append = append.replace(
                "datacopy", instance.codigo
            )
...
class CortesiaInLine(admin.TabularInline):
    ...
    fields = ["codigo", "aluno", "email", "utilizado"]


@admin.register(CheckoutItens)
class CheckoutItensAdmin(admin.ModelAdmin):
    raw_id_fields = ["curso"]
    search_fields = ["curso__nome", "checkout__code", "checkout__aluno__nome"]
    list_display = ("checkout", "curso")
    list_filter = [
...
class AtividadeForm(ModelForm):
    class Meta:
        widgets = {"enunciado": LinkedSelect}
    ...
    def response_change(self, request, obj):
        if request.POST.get("_lista_emails"):
            ...
            response["Content-Disposition"] = (
                'attachment; filename="lista-{0}.csv"'.format(slugify(obj))
            )
...
class TarefaAtividadeForm(ModelForm):
    class Meta:
        widgets = {
            ...
            "enunciado": RedactorEditor(),
            "padrao_resposta": RedactorEditor(),
...
class AtividadeAdmin(admin.ModelAdmin):
        ...
        (
            None,
            {
                ...
                "fields": [
                    ...
                    "data_ini",
                    "data_fim",
                    "resposta_padrao_data",
                    "resolucao_obrigatorio",
                    "video_id",
                ],
            },
        ),
        (None, {"classes": ("suit-tab", "suit-tab-tarefa"), "fields": ["tarefa"]}),
        (None, {"classes": ("suit-tab", "suit-tab-gabarito"), "fields": ["gabarito"]}),
        (
            None,
            {
    ...
    def _copy(self, request, pk):
        ...
        for modelo in atividade.atividademodelo_set.all():
            modelo = AtividadeModelo(atividade=atividade_cp, arquivo=modelo.arquivo)
            modelo.save()
        formulario = getattr(atividade, "formulario", None)
        if formulario:
            messages.info(request, "Formulario ID: [{0}]".format(formulario.pk))
        messages.success(
            request, "Atividade [{}], Duplicada com sucesso.".format(atividade_cp)
        )
...
class TarefaAtividadeAdmin(admin.ModelAdmin):
    ...
    list_display = ["atividade", "aluno", "concluido", "tempo", "corrigido"]
...
class CursoAdmin(AdminImageMixin, SortableModelAdmin):
    ...
    fieldsets = [
        ...
        (None, {"classes": ("suit-tab", "suit-tab-modulos"), "fields": []}),
        (None, {"classes": ("suit-tab", "suit-tab-material"), "fields": []}),
        (None, {"classes": ("suit-tab", "suit-tab-discussao"), "fields": []}),
        (
            None,
            {"classes": ("suit-tab", "suit-tab-cronograma"), "fields": ["cronograma"]},
        ),
        (None, {"classes": ("suit-tab", "suit-tab-mural"), "fields": ["mural"]}),
        (
            None,
            {
        ...
        (None, {"classes": ("suit-tab", "suit-tab-seo"), "fields": ["saiba_mais"]}),
        (
            None,
            {
        ...
        (None, {"classes": ("suit-tab", "suit-tab-cortesia"), "fields": []}),
    ]
    suit_form_tabs = (
        ("geral", "Geral"),
        ("modulos", "Módulos"),
    ...
    def subscribe_aluno(self, request, queryset):
        ...
        self.message_user(
            request,
            "{0} de {1} Aluno(s) foram inscritos no Newsletter.".format(inc, total),
        )
    ...
    def _copy(self, request, pk):
        ...
        curso_cp.nome = "{} - Cópia".format(curso.nome)
        ...
        messages.success(request, "{0}, Duplicado com sucesso.".format(curso_cp))
...
class SimuladoAdmin(AdminImageMixin, SortableModelAdmin):
    ...
    suit_form_includes = (("curso/admin/cortesia_info.html", "top", "cortesia"),)
    ...
    def _copy(self, request, pk):
        ...
        messages.success(
            request, "Simulado {}, Duplicado com sucesso.".format(simulado_cp.pk)
        )
...
class ModuloAdmin(SortableModelAdmin):
    ...
    fieldsets = [
        ...
        (None, {"classes": ("suit-tab", "suit-tab-videos"), "fields": []}),
        (None, {"classes": ("suit-tab", "suit-tab-pdf"), "fields": []}),
    ]
    suit_form_tabs = (("geral", "Geral"), ("videos", "Videos"), ("pdf", "Arquivos PDF"))
    ...
    def _copy(self, request, pk):
        ...
        messages.success(
            request, "Modulo {}, Duplicado com sucesso.".format(modulo_cp.pk)
        )
...
class SentencaAvulsaAdmin(admin.ModelAdmin):
    ...
    def _copy(self, request, pk):
        ...
        sentenca_cp.id = None
        sentenca_cp.titulo = "{} - Cópia".format(sentenca.titulo)
        ...
        messages.success(request, "{0}, Duplicado com sucesso.".format(sentenca.titulo))
