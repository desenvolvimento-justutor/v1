...
class MetaFieldsForm(ModelForm):
    class Meta:
        widgets = {
            "meta_description": EnclosedInput(
                prepend="icon-comment", attrs={"class": "input-xxlarge"}
            ),
            "meta_keywords": EnclosedInput(
                prepend="icon-tag", attrs={"class": "input-xxlarge"}
            ),
...
class ConfiguracaoAdmin(AdminImageMixin, admin.ModelAdmin):
    ...
    fieldsets = [
        ...
        (None, {"classes": ("suit-tab suit-tab-imagem ",), "fields": []}),
        (
            None,
            {
        ...
        (None, {"classes": ("suit-tab suit-tab-descontos",), "fields": []}),
    ]
...
class BannerAdmin(AdminImageMixin, SortableModelAdmin):
    ...
    sortable = "order"
    list_display = ("titulo", "imagem", "ativo_inicio", "ativo_fim", "order")
    list_editable = ("order",)
    fieldsets = (
        (
            None,
        ...
        (None, {"fields": ("banner", "link_v2")}),
        (None, {"fields": ("ativo_inicio", "ativo_fim")}),
    )
...
class InstitucionalFormAdmin(ModelForm):
    ...
    class Media:
        ...
        css = {"all": ("filebrowser/css/suit-filebrowser.css",)}
...
@admin.register(GPTMModels)
class GPTMModelsAdmin(admin.ModelAdmin):
    list_display = ("model",)
    search_fields = ("model",)
