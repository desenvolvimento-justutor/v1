...
from .models import Nota, Formulario, Tabela, TabelaCorrecaoAluno, TabelaAluno, RespostaAluno
...
class FormularioAdmin(admin.ModelAdmin):
    ...
    raw_id_fields = ['enunciado']
    list_display = ['titulo', 'enunciado', 'creditos']
    search_fields = ['titulo', 'texto', 'enunciado']

    def _copy(self, request, pk):
        formulario = Formulario.objects.get(pk=pk)
        formulario_cp = copy.copy(formulario)
        formulario_cp.id = None
        formulario_cp.enunciado = None
        formulario_cp.titulo = u'[COPY {}] {}'.format(formulario.id, formulario.titulo)
        formulario_cp.save()
        ...
        return redirect('/admin/formulario_auto_correcao/formulario/%s/' % formulario_cp.id)
