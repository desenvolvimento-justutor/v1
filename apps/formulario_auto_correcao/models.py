...
from apps.enunciado.models import EnunciadoProposta
...
class Nota(models.Model):
    ...
    class Meta:
        verbose_name = u'Nota padrão'
        verbose_name_plural = u'Notas padrão'
...
class Formulario(models.Model):
    enunciado = models.OneToOneField(
        verbose_name='Enunciado', to=EnunciadoProposta, on_delete=models.PROTECT,
        related_name='formulario_autocorrecao'
    )
    titulo = models.CharField(
        verbose_name='Título', max_length=200
    )
    ...
    professor = models.ForeignKey(
        to=Professor, on_delete=models.PROTECT,
        null=True, blank=True
    )
    desabilitar = models.BooleanField(
        default=False
    )
    creditos = models.PositiveSmallIntegerField(
        verbose_name=u'Créditos', blank=True, null=True
    )
...
class Tabela(models.Model):
    formulario = models.ForeignKey(
        verbose_name='Formulário', to=Formulario, on_delete=models.CASCADE,
        null=True, blank=True, related_name='autocorrecao_tabelas'
    )
...
class TabelaCorrecaoAluno(models.Model):
    ...
    professor = models.ForeignKey(
        ...
        blank=True, null=True, related_name='autocorrecao_professores'
    )
    aluno = models.ForeignKey(
        verbose_name='Aluno', to=Aluno, on_delete=models.PROTECT,
        related_name='autocorrecao_alunos'
    )
    formulario = models.ForeignKey(
        verbose_name='Formulário', to=Formulario, on_delete=models.CASCADE,
        related_name='autocorrecao_formularios_aluno'
    )
    texto = models.TextField(
...
class RespostaAluno(models.Model):
    tabela = models.ForeignKey(
        verbose_name='Tabela', to=Tabela, on_delete=models.CASCADE,
        related_name='autocorrecao_tabela_resposta'
    )
    ...
    aluno = models.ForeignKey(
        verbose_name='Aluno', to=Aluno, on_delete=models.PROTECT,
        related_name='autocorrecao_alunos_resposta'
    )